...
function kube::codegen::gen_helpers() {
    ...
    if [ "${#input_pkgs[@]}" != 0 ]; then
        ...
        "${gobin}/deepcopy-gen" \
            ...
            --output-package heterflow/api/v1beta1 \
            --go-header-file "${boilerplate}" \
            "${input_pkgs[@]}"
    fi
    
    # for element in "${input_pkgs[@]}"
    # do
    #     echo "$element"
    # done
    echo "deepcopy ${#input_pkgs[@]}"
    # Defaults
    #
    local input_pkgs=()
    while read -r dir; do
    ...
    echo "in kube::codegen::gen_helpers"
}
...
function kube::codegen::gen_openapi() {
    ...
    local out_pkg=""
    local extra_pkgs=()
    local report="/mnt/data/nfs/myx/heterflow/tmp/report"
    local update_report=""
    local boilerplate="${KUBE_CODEGEN_ROOT}/boilerplate.go.txt"
    local v="${KUBE_VERBOSE:-0}"
    ...
    echo "in kube::codegen::gen_openapi in_dir ${in_dir} out_dir ${out_dir} out_pkg ${out_pkg} boilerplate ${boilerplate} gobin ${gobin} v ${v} input_pkgs ${input_pkgs[@]}"
}
...
function kube::codegen::gen_client() {
    ...
    # echo "in kube::codegen::gen_client in_dir ${in_dir} out_dir ${out_dir} out_pkg ${out_pkg} boilerplate ${boilerplate} gobin ${gobin} v ${v} input_pkgs ${inputs[*]}"

}

kube::codegen::gen_client /mnt/data/nfs/myx/heterflow/api --output-dir heterflow/pkg/generated --output-pkg heterflow/pkg/generated
kube::codegen::gen_helpers ./heterflow/api
